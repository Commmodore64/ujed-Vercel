...
const Index = () => {
      ...
      if (isCard) {
        ...
        for (const course of coursesData) {
          const response = await fetch(
            `https://200.23.125.118/api/inscripciones/${course.id}`
          );
          if (response.ok) {
            const data = await response.json();
        ...
        const response = await fetch("https://200.23.125.118/api/adeudos");
        if (response.ok) {
          const data = await response.json();
          allInscriptions.push(...data);
        } else {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await fetch(
        `https://200.23.125.118/api/eliminarpago/${selectedReferencia}`,
        {
          method: "POST",
          headers: {
      ...
      const responseUpdate = await fetch(
        "https://200.23.125.118/api/actualizarpago",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleFileUpload = async () => {
    if (file) {
      ...
      try {
        const response = await fetch(
          "https://200.23.125.118/api/subir-archivo",
          {
            method: "POST",
            body: formData,
          }
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const responseAdeudos = await fetch("https://200.23.125.118/api/adeudos");
      const adeudosData = await responseAdeudos.json();
      console.log("Datos de adeudos:", adeudosData);

      // Hacer fetch a la API de pagos
      const responsePagos = await fetch("https://200.23.125.118/api/pagos");
  ...
  const fetchPagosNoConciliados = async () => {
    try {
      const response = await fetch(
        "https://200.23.125.118/api/pagosnoconciliados"
    ...
    try {
      ...
      if (adeudosData.length === 0) {
        ...
        const responseAdeudos = await fetch(
          "https://200.23.125.118/api/adeudos"
        );
